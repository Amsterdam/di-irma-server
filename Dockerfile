...
RUN wget -q -O /usr/local/bin/irma https://github.com/privacybydesign/irmago/releases/download/v0.12.1/irma-linux-amd64
RUN chmod +x /usr/local/bin/irma
COPY ./config ./config
CMD irma server -v --config $CONFIG --url $BASE_URL --sse --no-emaill
EXPOSE 8088
